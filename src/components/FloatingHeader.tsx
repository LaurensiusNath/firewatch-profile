...
const FloatingHeader = () => {
  ...
  if (!isAuthenticated) {
    return null; // Don't show header on sign-in page
  }
    ...
    <>
      <header
        className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-300 ${
          isScrolled
            ? "w-11/12 max-w-6xl bg-white/90 backdrop-blur-md shadow-lg"
            : "w-full max-w-7xl bg-white/80 backdrop-blur-sm"
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Badak LNG</p>
              </div>
            </div>
              ...
              <div className="flex flex-col space-y-2">
                {navItems.map((item) => (
                  <button
                    key={item.name}
                    onClick={() => scrollToSection(item.href)}
                    className="text-left px-3 py-2 text-sm font-medium text-gray-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors duration-200"
                  >
                    {item.name}
                  </button>
                ))}
